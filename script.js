...
function aktipesan2() {
  ...
  new TypeIt("#kalimat", {
    ...
    lifelike: true,
    afterComplete: function () {
      setTimeout(aktipesan01, 2000);
    },
  }).go();
}
vketik01 = pesan01.innerHTML;
function aktipesan01() {
...
function aktipesan4() {
  wallpaper.style = "transform: scale(1.5);";
  kalimat.innerHTML = pesan4.innerHTML + " " + nama + " sayang 🥳❤️";
...
function aktipesan7() {
  ...
  new TypeIt("#pesan7", {
    ...
    afterComplete: function () {
      ...
      fungsi = 1;
      setTimeout(aktipesan20, 800);
    },
  }).go();
}
vketik20 = pesan20.innerHTML;
pesan20.innerHTML = "";
function aktipesan20() {
  fthilang();
  ftganti = 1;
  setTimeout(ftmuncul, 300);
  wallpaper.style = "transform: scale(1);";
  new TypeIt("#pesan20", {
    ...
    lifelike: true,
    afterComplete: function () {
      pesan20.innerHTML = vketik20;
      fungsi = 1;
      setTimeout(aktipesan02, 800);
    },
  }).go();
}
vketik02 = pesan02.innerHTML;
pesan02.innerHTML = "";
function aktipesan02() {
  fthilang();
  ftganti = 1;
  setTimeout(ftmuncul, 300);
  wallpaper.style = "transform: scale(1);";
  new TypeIt("#pesan02", {
    ...
    lifelike: true,
    afterComplete: function () {
      pesan02.innerHTML = vketik02;
      fungsi = 1;
      setTimeout(aktipesan03, 800);
    },
  }).go();
}
vketik03 = pesan03.innerHTML;
pesan03.innerHTML = "";
function aktipesan03() {
  fthilang();
  ftganti = 1;
  setTimeout(ftmuncul, 300);
  wallpaper.style = "transform: scale(1);";
  new TypeIt("#pesan03", {
    ...
    lifelike: true,
    afterComplete: function () {
      pesan03.innerHTML = vketik03;
      fungsi = 1;
      setTimeout(aktipesan04, 800);
    },
  }).go();
}
vketik04 = pesan04.innerHTML;
pesan04.innerHTML = "";
function aktipesan04() {
  fthilang();
  ftganti = 1;
  setTimeout(ftmuncul, 300);
  wallpaper.style = "transform: scale(1);";
...
function aktipesan8() {
  ...
  pesan20.style = "display:none";
  pesan02.style = "display:none";
  pesan03.style = "display:none";
  pesan04.style = "display:none";
  pesan8.style = "";
  wallpaper.style = "transform: scale(1);";
  ftganti = 4;
  ftmuncul();
...
document.getElementById("lv9").onclick = function () {
  lv9.style = "opacity:0";
  slov += 1;
  this.onclick = null;
  checkslov();
};
document.getElementById("lv10").onclick = function () {
  lv10.style = "opacity:0";
  slov += 1;
  this.onclick = null;
  checkslov();
};
...
async function pertanyaan() {
  ...
  if (prtanya) {
    ...
    // Konfirmasi dengan tombol "Ya" dan "Tidak"
    var { isConfirmed: lanjutTindakan } = await Swal.fire({
      title: nama + " " + confirm1,
      text: "" + opskalimat,
      imageUrl: "" + fotostiker6.src,
      showCancelButton: true,
      confirmButtonText: "" + tompositif1,
      cancelButtonText: "" + tomnegatif,
    });

    if (lanjutTindakan) {
      // Efek "tidak bisa dipilih" pada tombol "Ya"
      var buttonYes = document.querySelector(".swal2-confirm");
      buttonYes.style.position = "absolute";
      buttonYes.style.left = Math.random() * 100 + "px";
      buttonYes.style.top = Math.random() * 100 + "px";
